 ...
 const currentUser = useSelector((state) => state.user?.user);
  ...
  >
    ...
    <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
      <div><span className="font-bold text-[#34495E]">📧 Email :</span> {user?.email}</div>
      <div><span className="font-bold text-[#34495E]">📞 Téléphone :</span> {user?.tel}</div>
      <div><span className="font-bold text-[#34495E]">🎂 Âge :</span> {user?.age}</div>
      <div><span className="font-bold text-[#34495E]">📍 Origine :</span> {user?.gouvernorat}</div>
      <div><span className="font-bold text-[#34495E]">🏠 Adresse actuelle :</span> {user?.adresse}</div>
    </div>
